...
def cancel_order(current_user, order_id):
    ...
    try:
        ...
        order.cancel_order(request.json.get(\\'reason\\'))
...
def get_categories(current_user):
    ...
    try:
        ...
        return create_success_response({\\'categories\\': categories_data})
    except Exception as e:
...
def create_category(current_user):
    ...
    try:
        if Category.query.filter_by(name=data[\\'name\\']).first():
            return create_error_response(\\'DUPLICATE_CATEGORY\\', \\'Category with this name already exists\\', status_code=409)
...
def update_category(current_user, category_id):
    ...
    try:
        ...
        if not category:
            return create_error_response(\\'CATEGORY_NOT_FOUND\\', \\'Category not found\\', status_code=404)
        
        if Category.query.filter(Category.name == data[\\'name\\'], Category.category_id != category_id).first():
            return create_error_response(\\'DUPLICATE_CATEGORY\\', \\'Category with this name already exists\\', status_code=409)
        ...
        db.session.rollback()
        return create_error_response(\\'UPDATE_FAILED\\', f\\'Failed to update category: {str(e)}\\', status_code=500)
...
def delete_category(current_user, category_id):
    ...
    try:
        ...
        db.session.rollback()
        return create_error_response(\\'DELETE_FAILED\\', f\\'Failed to delete category: {str(e)}\\', status_code=500)
